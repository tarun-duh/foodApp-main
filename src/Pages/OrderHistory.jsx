    ...
    <div className="p-6 max-w-3xl mx-auto bg-gray-900 min-h-screen text-white">
      ...
      {loading ? (
        <ul className="space-y-4">
          {[...Array(3)].map((_, i) => (
            <li key={i} className="bg-gray-800 p-4 rounded shadow animate-pulse">
              ...
              {expandedOrderId === order.id && (
                <div className="mt-4 border-t border-gray-700 pt-4 text-sm text-gray-300">
                  ...
                  <div className="mt-2">
                    ...
                    <ul className="list-disc ml-6 mt-1">
                      {order.items?.map((item, idx) => (
                        <li key={idx}>
                          <span className={item.isVeg ? "text-green-400" : "text-red-400"}>
                            ●
                          </span>{" "}
                  ...
                  {order.createdAt?.seconds && (
                    <div className="mt-2">
                      <strong>Created At:</strong> {new Date(order.createdAt.seconds * 1000).toLocaleString()}
                    </div>
                  )}
                </div>
              )}
